...
if(typeof BX.Crm.EntityDetailProgressStep === "undefined")
{
	...
	BX.Crm.EntityDetailProgressStep.prototype =
	{
		...
		onClick: function(e)
		{
            ...
            var res = 0;
            var self = this;
            if(this._settings.control._entityTypeId == 2) {
                if (this._id == 3 || this._id == 'WON' || this._id == 'C2:3' || this._id == 'C2:WON') {
                    //console.log(this);
                    BX.ajax({
                        url: '/local/ajax/check_deal_fields.php',
                        data: {'id': this._settings.control._entityId}, // тут значения
                        method: 'POST',
                        dataType: 'json',
                        timeout: 30,
                        async: true,
                        ...
                        onsuccess: function (data) {
                            ...
                            res = data;
                            if (data == 0) {
                                alert('Выберите товар, не забудьте указать дату завершения');
                            } else {
                                self._control.processStepSelect(self);
                            }
                        },
                        onfailure: function () {
