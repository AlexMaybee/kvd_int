...
foreach ($arResult['PRODUCTS'] as $productId => $arItems)
{
    ...
    if(!empty($arResult['ADDIT_STORE_PRODUCT'])) {
        foreach ($arResult['ADDIT_STORE_PRODUCT'] as $addit_store) {
            $res_quan[] = getStoreCountProduct($arItems['ID'], $addit_store['ID']);
        }
    }

    //dg($res_quan);

    if(array_sum($res_quan) > 0) {
        $arActions = array();
        if ($arItems['TYPE'] === 'S')
        {
            $arActions[] = array(
        ...
        if(!empty($arResult['ADDIT_STORE_PRODUCT'])) {
            foreach ($arResult['ADDIT_STORE_PRODUCT'] as $addit_store) {
                $gridDataRecord['columns']['STORE_'.$addit_store['ID']] = getStoreCountProduct($arItems['ID'], $addit_store['ID']);
            }
        }
