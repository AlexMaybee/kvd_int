...
//dg($arResult['ELEMENT_ID']);
?>

<script>
    var store_id = '<?=$arResult['ELEMENT_ID'];?>';
</script>
<?

$APPLICATION->AddHeadScript('/local/components/bitrix/lists.element.edit/templates/.default/script_custom.js');
...
foreach($arResult["FIELDS"] as $fieldId => $field)
{
	...
	if($preparedData)
	{
        ...
        $name_f = explode('|', $preparedData['name']);
        if (LANGUAGE_ID == 'en'){
            $preparedData['name'] = ($name_f[1]) ? trim($name_f[1]) : trim($name_f[0]);
        } else {
            $preparedData['name'] = trim($name_f[0]);
        }
...
if(CModule::IncludeModule("bizproc") && CBPRuntime::isFeatureEnabled() && $arResult["IBLOCK"]["BIZPROC"] != "N")
{
    ...
    if($arResult['ELEMENT_ID'] > 0 && $arResult["IBLOCK"]['ID'] == 31) {
