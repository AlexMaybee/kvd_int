...
)
{
	...
	)
	{
		...
		if($bBizproc)
		{
			...
			{
			    if(CSite::InGroup( array(18, 19, 20, 21) )) {
                    $canWrite = true;
                } else {
		...
		if(!$strError)
		{
			...
			if(isset($_POST["save"]))
			{
			    if($arResult['ELEMENT_FIELDS']['IBLOCK_ID'] == 31){
                    LocalRedirect('/services/lists/'.$arResult['ELEMENT_FIELDS']['IBLOCK_ID'].'/view/0/?list_section_id='.$arResult['ELEMENT_FIELDS']['IBLOCK_SECTION_ID']);
                } else {
                    LocalRedirect($arResult["~LIST_SECTION_URL"]);
                }
			}
