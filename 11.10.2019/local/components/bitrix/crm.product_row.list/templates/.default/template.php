...
$display_b = 'none';
$stage_id = CCrmDeal::GetByID($arResult['OWNER_ID']);
if($stage_id['STAGE_ID'] == 3 || $stage_id['STAGE_ID'] == 'WON' || $stage_id['STAGE_ID'] == 'C2:3' || $stage_id['STAGE_ID'] == 'C2:WON') {
    $display_b = 'none';
} else {
    $display_b = '';
}
	...
	<table id="<?= $productContainerID ?>" class="crm-items-table" style="<?= $nProductRows === 0 ? 'display: none;' : '' ?>">
		<thead>
			<tr class="crm-items-table-header">
				<td class="crm-item-cell crm-item-name"><span class="crm-item-cell-text"><?=htmlspecialcharsbx(GetMessage('CRM_PRODUCT_ROW_COL_TTL_NAME')).' / '.htmlspecialcharsbx(GetMessage('VIN_CODE'))?></span></td>
		...
		for ($i = 0; $i < $nProductRows; $i++)
		{
            ...
            $db_props = CIBlockElement::GetProperty(27, $row['PRODUCT_ID'], array("sort" => "asc"), Array("ID"=> 105));
			...
			<tr id="<?=$rowID?>" class="<?=$className?>">
				<td class="crm-item-cell crm-item-name">
                    <table>
                        ...
                        <tr>
                            ...
                            <td><?echo ($ar_props["VALUE"]) ? $ar_props["VALUE"] : '';?></td>
                        </tr>
                        </tbody>
                    </table>
				</td>
				...
				<td class="crm-item-cell crm-item-move" style="display: <?echo $display_b?>"><span class="crm-item-del"<?= $bInitEditable ?  '' : ' style="display: none;"' ?>></span></td>
			</tr>
		<?}?>
		</tbody>
	</table>
