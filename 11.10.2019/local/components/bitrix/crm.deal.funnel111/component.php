...
$obRes = CCrmDeal::GetListEx(array(), $arFilter, false, false, array('ID', 'STAGE_ID', 'OPPORTUNITY', 'CURRENCY_ID', 'COMPANY_ID', 'CONTACT_ID'));
...
while($arDeal = $obRes->Fetch())
{
	...
	//Добавляем поля Контакта и Компании, и подсчитываем к-во
	if( $arDeal['COMPANY_ID'] > 0 ) {
        $arResult['COMPANY_COUNT'][$stageID]['COMPANY']++;
    }

    if( $arDeal['CONTACT_ID'] > 0 ) {
        $arResult['CONTACT_COUNT'][$stageID]['CONTACT']++;
    }
...
foreach ($arResult['STAGE_LIST'] as $k => $v)
{
	...
	$info['COMPANY_RESULT'] = ($arResult['COMPANY_COUNT'][$k]['COMPANY'] > 0 ? $arResult['COMPANY_COUNT'][$k]['COMPANY'] : 0);
	$info['CONTACT_RESULT'] = ($arResult['CONTACT_COUNT'][$k]['COMPANY'] > 0 ? $arResult['CONTACT_COUNT'][$k]['CONTACT'] : 0);
	$info['TITLE'] = $v;
	$info['STAGE_ID'] = $k;
