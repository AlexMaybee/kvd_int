...
while($arSection = $rsSections->GetNext())
{
    ...
    //"NAME" => ($arIBlock["ID"] == 31) ? getNameSectionEn(31, $arSection["NAME"], 'UF_NAME_EN') : $arSection["NAME"],
...
while($obElement = $rsElements->GetNextElement())
{
    ...
    $section_id_custom = getElementSectionID($data["ID"], $data['IBLOCK_ID']);
	//dg($section_id_custom);
	//dg($data);
	if($_GET['list_section_id'] > 0 && $data['IBLOCK_ID'] == 31 && $_GET['list_section_id'] == $section_id_custom/*$data['IBLOCK_SECTION_ID']*/) {
        $arResult["ELEMENTS_ROWS"][] = array(
            "id" => $data["ID"],
            "data" => $data,
            "actions" => $aActions,
