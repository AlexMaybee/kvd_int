...
if (is_array($arResult['HEADERS']))
{
	...
	$additHeaderStore = checkPermissionSection();

	if(!empty($additHeaderStore)) {
        foreach ($additHeaderStore as $head)
        {
            $newHeaders[] = [
                'id' => 'STORE_'.$head['ID'],
                'sort' => 'STORE_'.$head['ID'],
        ...
        $arResult['ADDIT_STORE_PRODUCT'] = $additHeaderStore;
    }
...
function checkPermissionSection()
{
    $result = [];
    ...
    while($ar_result = $db_list->GetNext())
    {
        ...
        if ($arParentSection = $rsParentSection->GetNext())
        {
            ...
            {
                $result[] = ['ID'=> $arSect['ID'], 'NAME'=> $arSect['NAME']];
            }
        }
...
function getStoreCountProduct($ID, $SECTION_ID)
{
    ...
    if($arFields['PROPERTY_KOLICHESTVO_VALUE']) {
        return $arFields['PROPERTY_KOLICHESTVO_VALUE'];
    } else {
        return 0;
    }
}