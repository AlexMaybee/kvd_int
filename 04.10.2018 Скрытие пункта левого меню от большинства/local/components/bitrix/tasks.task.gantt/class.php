...
class TasksTaskGanttComponent extends TasksTaskListComponent
{
	...
	protected function getData()
	{
		...
		// TODO ������� ���� ��
        //print_r($this->arResult[ 'LIST' ]);
        foreach ($this->arResult[ 'LIST' ] as $key=> $item)
        {
            $check = getCheckList($item[ 'ID' ]);
            if($check['LIST'] > 0) {
                $item['TITLE'] = $item['TITLE'] . ' (' . $check['CHECKED'] .'/'. $check['LIST'] . ')';
                $this->arResult['LIST'][$key] = $item;
            }
