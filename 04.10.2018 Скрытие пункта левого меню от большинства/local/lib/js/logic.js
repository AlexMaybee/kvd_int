	...
	BX.Tasks.Component.TasksWidgetCheckList.ItemManager = BX.Tasks.Util.ItemSet.extend({
		...
		methods: {
			...
			onItemToggleChange: function(item)
			{
                ...
                if(item.sys.parent.opts.entityId > 0 && item.value() > 0 && item.isChecked() == false) {
                    console.log(item.value());
                    console.log(item.opts.data.TITLE);
                    console.log(item.sys.parent.opts.entityId);
                    BX.ajax({
                        url: '/local/ajax/update_task_stage.php',
                        data: {'id': item.sys.parent.opts.entityId, 'status_name': item.opts.data.TITLE},
                        method: 'POST',
                        dataType: 'json',
                        timeout: 30,
                        async: true,
